...
#layout = [{'col': x, 'row': y, 'x': x, 'y': y} for y,x in itertools.product(range(4), range(12))]
#json.dump(layout, open('flat-layout.json', 'w'))

#+end_src

* Run this
keymap -c keymap_drawer.config.yaml parse -c 12 -z config/andean-condor.keymap > /tmp/foo.yaml && keymap -c keymap_drawer.config.yaml draw --qmk-info-json=flat-layout.json /tmp/foo.yaml > /tmp/foo.svg 
